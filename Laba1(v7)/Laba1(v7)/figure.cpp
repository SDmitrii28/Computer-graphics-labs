...
void Figure::Draw(Graphics &g)
{
	...
	//g.SetPixel(vertices_[5][0],vertices_[1][1], RGB(0, 200, 0));

	
}
void Figure::DrawPoint(Graphics& g)
{
	/*std::vector<std::array<float, 2>> points(2);
	points[0][0] = vertices_[0][0];
	points[0][1] = vertices_[0][1];
	points[1][0] = vertices_[0][0] + 10;
	points[1][1] = vertices_[0][1] + 10;
	points[2][0] = vertices_[2][0] + 5;
	points[2][1] = vertices_[2][1] + 5;*/
	for (auto e : edges_)
	{
		g.DrawLine(vertices_[e[0]][0], vertices_[e[0]][1], vertices_[e[1]][0], vertices_[e[1]][1], RGB(255, 255, 255));
	}
